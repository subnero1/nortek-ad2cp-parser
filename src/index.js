...
const df3CurrentProfileData = new Parser()
  ...
  .uint16le("blankingDistance", {
    formatter: function (value) {
      return (
        value * (this["status"]["blankingDistanceScalingInCm"] ? 0.01 : 0.001)
      );
    },
  });
...
const df3VelocityData = new Parser()
  ...
  .array("velocityData", {
    ...
    length: function () {
      return (
        this["numberOfBeams"] * this["numberOfCells"] * this["hasVelocityData"]
      );
    },
  ...
  .array("amplitudeData", {
    ...
    length: function () {
      return (
        this["numberOfBeams"] * this["numberOfCells"] * this["hasAmplitudeData"]
      );
    },
    formatter: function (values) {
...
const df7CurrentProfileData = new Parser()
  ...
  .array("velocityData", {
    ...
    length: function () {
      return (
        this["numberOfBeams"] * this["numberOfCells"] * this["velocityIncluded"]
      );
    },
    formatter: function (values) {
        ...
        if (types.includes(type)) {
          ...
          if (type === "PNORS") {
            sentence = sentence.filter(
              (_, index) => ![7, 10, 12, 14].includes(index),
            );
          }
        ...
        if (types.includes(type)) {
          ...
          for (let i = 0; i < record.numberOfCells; i++) {
            let sentence = [
              ...
              (tag ? "CN=" : "") + (i + 1).toString(),
              (tag ? "CP=" : "") +
                (record.blankingDistance + (i + 1) * record.cellSize).toFixed(
                  1,
                ),
              velocityTags[0] +
                record.velocityData[0 * record.numberOfCells + i].toFixed(3),
              velocityTags[1] +
                record.velocityData[1 * record.numberOfCells + i].toFixed(3),
              velocityTags[2] +
                record.velocityData[2 * record.numberOfCells + i].toFixed(3),
              velocityTags[3] +
              ...
              (tag ? "A1=" : "") +
                record.amplitudeData[0 * record.numberOfCells + i].toFixed(1),
              (tag ? "A2=" : "") +
                record.amplitudeData[1 * record.numberOfCells + i].toFixed(1),
              (tag ? "A3=" : "") +
                record.amplitudeData[2 * record.numberOfCells + i].toFixed(1),
              (tag ? "A4=" : "") +
                (record.numberOfBeams == 4
              ...
              (tag ? "C1=" : "") +
                record.correlationData[0 * record.numberOfCells + i],
              (tag ? "C2=" : "") +
                record.correlationData[1 * record.numberOfCells + i],
              (tag ? "C3=" : "") +
                record.correlationData[2 * record.numberOfCells + i],
              (tag ? "C4=" : "") +
                (record.numberOfBeams == 4
    ...
    if (record.dataSeriesId == 0x30) {
      ...
      if (types.includes("PNORE")) {
        sentences.push([
          ...
          ...record.energySpectrum.data.map((value) =>
            (value * 1e-4).toFixed(3),
          ),
          //                                                    ^^^^ undocumented
        ]);
      }
        ...
        for (const [flagIndex, flag] of ["MD", "DS"].entries()) {
          sentences.push([
            ...
            ...record.direction.data
              .slice(
                flagIndex * record.direction.nBins,
                (flagIndex + 1) * record.direction.nBins,
              )
              .map((value) => value.toFixed(4)),
          ]);
        }
